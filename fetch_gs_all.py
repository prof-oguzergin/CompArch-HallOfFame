"""
Fetch Google Scholar h-index, i10-index, total citations, and citation
buckets (100+, 200+, 400+, 800+, 1000+) for all 259 HoF members.

Pipeline per author:
1. DuckDuckGo search for their GS profile
2. Scrape profile page for h-index, i10, total cites
3. Paginate through sorted-by-cited paper list, count buckets
"""
...
def count_buckets(gs_id, thresholds=(100, 200, 400, 800, 1000)):
    """Fetch pages sorted by citations desc, stop when min threshold is crossed."""
...
print(f'Loaded {len(members)} HoF members', file=sys.stderr)

cache_file = 'gs_cache.json'
cache = {}
...
# Process
for i, (name, aff) in enumerate(members):
    if name in cache and cache[name].get('metrics'):
        continue
    ...
    if (i+1) % 5 == 0:
        ...
        print(f'  [{i+1}/{len(members)}] {ok} with metrics', file=sys.stderr)
    time.sleep(3)
...
ok = sum(1 for v in cache.values() if v.get('metrics'))
print(f'\nDONE: {ok}/{len(members)} have metrics', file=sys.stderr)
